...
export const useParamStore = defineStore('paramstore', {
  state: () => ({
    ...
    chain: {
      ...
      items: [
        {
          subtitle: 'params.height',
          icon: 'BoxIcon',
          color: 'light-success',
          value: '-',
        },
        {
          subtitle: 'params.bonded_and_supply',
          icon: 'DollarSignIcon',
          color: 'light-danger',
          value: '-',
        },
        {
          subtitle: 'params.bonded_ratio',
          icon: 'PercentIcon',
          color: 'light-warning',
          value: '-',
        },
        {
          subtitle: 'params.inflation',
          icon: 'TrendingUpIcon',
          color: 'light-primary',
          value: '-',
        },
      ...
      Promise.all([this.getStakingPool(), this.getBankTotal(bond_denom)]).then(
        (resArr) => {
          ...
          const bondedAndSupply = this.chain.items.findIndex(
            (x) => x.subtitle === 'params.bonded_and_supply'
          );
          ...
          const bondedRatio = this.chain.items.findIndex(
            (x) => x.subtitle === 'params.bonded_ratio'
          );
